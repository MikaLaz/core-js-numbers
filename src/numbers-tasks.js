...
function getAverage(value1, value2) {
  ...
  if (value1 + value2 >= Number.MAX_VALUE) {
    rezValue = value1 / 2 + value2 / 2;
  } else {
    rezValue = (value1 + value2) / 2;
  }
  return rezValue;
}
...
function roundToPowerOfTen(num, pow) {
  const power = 10 ** pow;
  const rez = Math.round(num / power) * power;
  return rez;
}
...
function isPrime(n) {
  ...
  for (let i = 2; i < Math.trunc(Math.sqrt(n)) + 1; i += 1) {
    if (n % i === 0) {
      return false;
    }
  }
...
function getFibonacciNumber(index) {
  ...
  for (let i = 0; i < arrFib.length; i += 1) {
    if (i === index) {
      retNum = arrFib[i];
    }
  }
...
function getCountOfOddNumbers(number) {
  let countOdd;
  if (number % 2 === 0) {
    countOdd = Math.abs(number / 2);
  } else {
    countOdd = Math.trunc(Math.abs(number / 2)) + 1;
  }
  return countOdd;
}
